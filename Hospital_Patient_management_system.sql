...
SELECT TOP 1 patient_name, date_of_birth
FROM Patients
...
SELECT AVG(DATEDIFF(DAY, admission_date, discharge_date)) AS avg_length_of_stay
FROM MedicalRecords;
...
SELECT TOP 1 doctor_id, COUNT(*) AS patient_count
FROM MedicalRecords
GROUP BY doctor_id
...
FROM Patients p
INNER JOIN MedicalRecords m ON p.patient_id = m.patient_id
ORDER BY DATEDIFF(DAY, m.admission_date, m.discharge_date) DESC;


-- 19. Display the top 5 most common diagnoses recorded in the medical records.

SELECT TOP 5 diagnosis, COUNT(*) AS diagnosis_count
FROM MedicalRecords
GROUP BY diagnosis
...
SELECT TOP 1
    p.patient_id,
    p.patient_name,
    DATEDIFF(DAY, MIN(m.admission_date), MAX(m.discharge_date)) AS total_days
FROM 
    Patients p
JOIN 
...
JOIN 
    ...
    AND DATEDIFF(DAY, m2.discharge_date, m1.admission_date) <= 30
JOIN 
...
FROM 
    MedicalRecords
WHERE 
    admission_date >= DATEADD(YEAR, -1, GETDATE())
GROUP BY 
...
WHERE 
    MedicalRecords.admission_date >= DATEADD(YEAR, -1, GETDATE())
GROUP BY 
...
SELECT 
    ...
    MONTH(admission_date) AS month,
    COUNT(*) AS admissions,
    ((COUNT(*) - LAG(COUNT(*), 1, 0) OVER (ORDER BY YEAR(admission_date), MONTH(admission_date))) * 100.0) / 
    NULLIF(LAG(COUNT(*), 1, 0) OVER (ORDER BY YEAR(admission_date), MONTH(admission_date)), 0) AS percentage_change
FROM 
    MedicalRecords
WHERE 
    admission_date >= DATEADD(YEAR, -1, GETDATE())
...
SELECT TOP 3 doctor_id, COUNT(*) AS patient_count
FROM MedicalRecords
GROUP BY doctor_id
ORDER BY patient_count DESC;
